...
.controller('projecttestCtrl', ['$scope', '$http', '$q', 'projecttestFactory', function ($scope, $http, $q, projecttestFactory) {

    // Import variables and functions from service
    $scope.data = projecttestFactory.data;
	$scope.items = projecttestFactory.items;
	$scope.detailsItem = projecttestFactory.detailsItem;
	
    // initialize the service
    projecttestFactory.init($scope);

	// Show main view in the first column
	$scope.portalHelpers.showView('projecttestMain.html', 1);
	
	// This function gets called when user clicks an item in the list
	$scope.showDetails = function(item){
		// Make the item that user clicked available to the template
		$scope.detailsItem.value = item;		
		$scope.portalHelpers.showView('projecttestDetails.html', 2);
	}
	
}])
// Factory maintains the state of the widget
.factory('projecttestFactory', ['$http', '$rootScope', '$filter', '$q', function ($http, $rootScope, $filter, $q) {
...
.directive('projecttestDirectiveName', ['$http', function ($http) {
	return {
...
.filter('projecttestFilterName', function () {
